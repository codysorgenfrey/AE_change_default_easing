            ...
            if (selectedKeys){
                for(selectedKey=0;selectedKey<selectedKeys.length;selectedKey++){
                    ...
                    if(keyInTempEase.length == 1){
                        layerProps[layerProp].setTemporalEaseAtKey(selectedKeys[selectedKey], [ease], [ease]);
                    } else if (keyInTempEase.length == 2){
                        layerProps[layerProp].setTemporalEaseAtKey(selectedKeys[selectedKey], [ease, ease], [ease, ease]);
                    } else {
                        layerProps[layerProp].setTemporalEaseAtKey(selectedKeys[selectedKey], [ease, ease, ease], [ease, ease, ease]);
                    }
                }
            }
        }
