        ...
        for(layerProp=0;layerProp<layerProps.length;layerProp++){
            ...
            if (selectedKeys){
                for(selectedKey=0;selectedKey<selectedKeys.length;selectedKey++){
                    var keyInTempEase = layerProps[layerProp].keyInTemporalEase(selectedKeys[selectedKey]);
                
                    if(keyInTempEase.length == 1){
                        layerProps[layerProp].setTemporalEaseAtKey(selectedKeys[selectedKey], [easeIn], [easeOut]);
                    } else if (keyInTempEase.length == 2){
                        layerProps[layerProp].setTemporalEaseAtKey(selectedKeys[selectedKey], [easeIn, easeIn], [easeOut, easeOut]);
                    } else {
                        layerProps[layerProp].setTemporalEaseAtKey(selectedKeys[selectedKey], [easeIn, easeIn, easeIn], [easeOut, easeOut, easeOut]);
                    }
                }
            }
        }
